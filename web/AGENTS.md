...
- Home catalog loads categories on a fast path (`sessionStorage` + `/api/vehicle-categories`) so filter chips render before the vehicle list finishes; vehicles load via paginated `GET /api/vehicles/models/search?page=&pageSize=12` (12 per page, one card per model line) with server-side filters
