...
- `ModelPage.tsx` — `/brand/:brandCode/models/:modelSlug`; hero (model + price) → compact year/trim bar → image + specs left, sticky confirm form right
...
- `AdminDataPage.tsx` — `/admin` easy forms; **admin role only** (sales operators see a forbidden message); left sidebar groups **Dữ liệu xe** and **Nhân sự** with expandable sub-lists (no top tab row); catalog and plate lists have live soft search plus tab-specific dropdown filters (vehicles: brand, category, body style, status; locations: region, fee zone; dealers/fee rules: brand or category). **Phụ kiện xe** tab edits accessory catalog (multilingual name, price, WebP photo stored in DB). **Bank loans** tab uses `BankLoanForm` with searchable bank and consultant pickers; manage reference data on **Banks** and **Consulting staff** tabs. Tab switches load only that tab’s list plus lookups the tab needs; `lib/adminCatalogCache.ts` deduplicates concurrent and repeat fetches until save/delete on that tab
...
- Each vehicle color can have multiple photos in `colorPhotos` (JSON array per color). Admin **Màu xe và ảnh** supports multi-upload, reorder, and remove per color. The vehicle-page slideshow shows only the selected color's photos. Upload converts to WebP in the browser first, then the API re-encodes with Sharp and saves the blob in the database. Quote and confirm pages use the first photo per color for thumbnails
