...
- `ColorPhotoImage.tsx` — vehicle color thumbnail; shows spinner overlay while the selected color image loads from `/api/vehicle-images/{id}` or an inline color swatch fallback
...
- `QuoteSheet.tsx` — renders the filled dealer Excel template from `src/server/assets/quote-report/`; scales to the same width as the quote panels above (no horizontal scroll); `#quote-sheet` is the PDF/PNG source; cell text is vertically centered with equal padding (not flex) so SVG capture matches the screen
...
- Quote sheet **CÁC MÀU XE** uses `QuoteColorGrid` + `ReportColorPhoto`; layout is a centered 2-column grid (`lib/colorGridLayout.ts` — 1 full frame, 2×N rows, odd last row centered); up to six colors; `GET /api/report-color-photo/[id]`
