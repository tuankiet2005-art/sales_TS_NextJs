...
function colorSwatchDataUri(name?: string): string {
  const hex = colorHex(name);
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64"><rect fill="${hex}" width="64" height="64"/></svg>`;
  return `data:image/svg+xml,${encodeURIComponent(svg)}`;
}

export function colorPhoto(name?: string, photos?: ColorPhotoMap | null): string {
  const fromVehicle = name ? firstResolvedColorPhoto(photos, name) : undefined;
  if (fromVehicle) {
    return fromVehicle;
  }
  return colorSwatchDataUri(name);
}
