...
import { accessoryLabel } from "@/features/shared/lib/labels";
...
import type { AccessoryCatalogItem, QuoteExtras } from "@onroad/shared/types";
        ...
        {catalog.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative rounded-xl border px-2.5 py-2 text-left ${
                selected ? "border-copper ring-1 ring-copper/30" : "border-ink/10 bg-white"
              }`}
            >
              {selected && (
                <span className="absolute right-1.5 top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-copper text-white">
                  <Check className="h-3 w-3" />
                </span>
              )}
              <p className="line-clamp-2 pr-5 text-[11px] font-semibold leading-4">{label}</p>
        ...
        <div className="mt-4 space-y-2 border-t border-ink/8 pt-3">
          {extras.accessories.map((item, index) => (
            <div
              ...
              className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 sm:grid-cols-[minmax(0,1fr)_7.5rem_auto]"
            >
              <input
                value={item.name}
                onChange={(event) => {
              ...
              <button
                ...
                className="inline-flex h-12 w-12 items-center justify-center rounded-xl border border-ink/10 text-ink/60 hover:bg-mist hover:text-red-700 sm:col-start-3 sm:row-start-1"
              ...
              <CurrencyInput
                ...
                className="col-span-2 h-12 w-full rounded-xl border border-ink/10 bg-paper px-3 text-base sm:col-span-1 sm:col-start-2 sm:row-start-1"
              />
            </div>
          ))}
        </div>
