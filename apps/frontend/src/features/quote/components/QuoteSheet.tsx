...
import type { VehicleDetail } from "@onroad/shared/types";
...
} from "@/features/quote/lib/quoteSheetCellLayout";
  ...
  return (
    <QuoteSheetScaler width={view.width} height={view.height}>
      <article
        ...
        className="relative box-border overflow-visible bg-white text-[#1f1f1f]"
              ...
              return (
                <tr key={r} style={{ height }}>
                  {view.columns.map((_, colIndex) => {
                    ...
                    const textAlign = quoteCellTextAlign(cell?.style?.textAlign, cell?.text);
                    const verticalAlign = quoteCellVerticalAlign(cell?.style?.verticalAlign);
                    const rowspan = cell?.rowspan ?? 1;
                    const cellHeight = quoteCellSpannedHeight(view.rows, rowIndex, rowspan);
                    const textStyle = quoteCellTextStyle(textAlign, cell?.style?.whiteSpace, rowspan, verticalAlign);
                    return (
                      <td
                        key={c}
                        colSpan={cell?.colspan}
                        rowSpan={cell?.rowspan}
                        style={tdStyle(cell?.style, textAlign, verticalAlign)}
                      >
                        <div
                          data-quote-cell-inner
                          style={quoteCellInnerStyle(
                            textAlign,
                        ...
                        >
                          <span data-quote-cell-text style={textStyle}>{cell?.text}</span>
                        </div>
                      </td>
                    );
                  })}
...
function tdStyle(
  style: QuoteSheetView["cells"][number]["style"] | undefined,
