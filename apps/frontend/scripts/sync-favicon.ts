...
async function main() {
  ...
  const iconRel = "apps/frontend/src/app/icon.png";
  const appleRel = "apps/frontend/src/app/apple-icon.png";

  run(`git add ${iconRel} ${appleRel}`);
  runQuiet("git add -u apps/frontend/src/app/favicon.ico");
...
chore(frontend): update OnRoad favicon from brand drop folder
