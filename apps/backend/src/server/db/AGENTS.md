...
- `repositories/app-settings.ts` — policy overrides in `app_settings`
- Catalog queries live in `src/server/modules/catalog/catalog.repository.ts` (not under `db/repositories/`)
