...
- Bottom merged cell receives a dynamic 1–6 photo grid overlay (`QuoteColorGrid` + `ReportColorPhoto`, `lib/colorGridLayout.ts` — centered 2-column rows) on screen; template car placeholders under **CÁC MÀU XE** are hidden. Word export composites the same layout via `quote-docx-fill.ts`. Excel export uses the template as-is.
...
- `npm test` from `apps/backend/` — `quote-sheet-fill.test.ts`
