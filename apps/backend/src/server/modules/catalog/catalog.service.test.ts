...
vi.mock("./catalog.repository", () => catalog);
...
describe("resolveQuoteCalculation", () => {
  ...
  it("merges accessories from the request into a reused breakdown", async () => {
    catalog.findActiveVehicleById.mockResolvedValue(vehicleRow);
    const stale = {
      ...breakdown,
      estimatedOnRoadTotal: 120,
      accessoriesTotal: 0,
      accessories: [],
    };
    const result = await resolveQuoteCalculation(
      {
        vehicleId: 13,
        locationId: 29,
        accessories: [{ name: "Bình chữa cháy", amount: 280000 }],
      },
      stale,
    );
    expect(result?.data.accessories).toEqual([{ name: "Bình chữa cháy", amount: 280000 }]);
    expect(result?.data.accessoriesTotal).toBe(280000);
    expect(result?.data.estimatedOnRoadTotal).toBe(120 + 280000);
  });
