...
import { formatQuoteAmount } from "@onroad/shared/format";
import type { QuoteSheetCellStyle, QuoteSheetView } from "@onroad/shared/quote/quoteSheetView";
...
export function worksheetToView(workbook: ExcelJS.Workbook, sheet: ExcelJS.Worksheet): QuoteSheetView {
  ...
  collapseEmptyAccessoryRows(cells, rows);
...
function mergedCellStyle(
  ...
  let borderRight = topRight.borderRight ?? bottomRight.borderRight ?? style.borderRight;
  if (!borderRight && sheetRight >= REPORT_COL_COUNT) {
    const belowRow = Math.min(sheetBottom + 1, sheet.rowCount);
    borderRight = cellStyle(sheet.getCell(belowRow, REPORT_COL_COUNT)).borderRight;
  }
  return {
    ...style,
...
function collapseEmptyGiftRows(cells: QuoteSheetView["cells"], rows: number[], firstSheetRow: number) {
  for (let sheetR = 13; sheetR <= 15; sheetR += 1) {
...
function collapseEmptyAccessoryRows(cells: QuoteSheetView["cells"], rows: number[]) {
  const header = cells.find((cell) => /CHI PHÍ PHÁT SINH THÊM/i.test(cell.text));
  const total = cells.find((cell) => /^TỔNG (CP|CHI PHÍ) PHÁT SINH/i.test(cell.text));
  if (!header || !total || total.r <= header.r) {
    return;
  }
  for (let r = header.r + 1; r < total.r; r += 1) {
    const hasText = cells.some((cell) => cell.r === r && cell.text.trim().length > 0);
    if (!hasText) {
      rows[r - 1] = 2;
    }
  }
